...
class Cosmos(private val connectionString: String) {
     ...
     */
    fun getDatabase(db: String): CosmosDatabase {
        check(db.isNotBlank()) { "db must be non-empty" }
        return CosmosDatabase(client, db)
    }
 ...
 *
 * Usage:
 * val account = CosmosAccount("AccountEndpoint=https://xxx.documents.azure.com:443/;AccountKey=xxx==;")
 * val db = account.getDatabase("Database1")
 *
 * //Then use db to do CRUD / query
...
class CosmosDatabase(private val client: DocumentClient, private val database: String) {
    ...
    protected val collectionMap: MutableMap<String, DocumentCollection> = mutableMapOf()
